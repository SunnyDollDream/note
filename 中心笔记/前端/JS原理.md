...
2. 创建绑定`this`的函数:
   3. bind:返回一个绑定了`this`的新函数,并不会立刻调用,而且传入的参数即为原方法中的参数,所以返回的新函数在调用时就不需要传bind时传入的参数了(按位置传入,如有a,b两个参数,bind(obj,1),调用时就只需要传入一个参数b了,a是刚才传入的1)
   4. 箭头函数:最近的this是谁,就是谁
...
student.sayHi()
```
>如果不使用箭头函数而是使用function关键字,那么this则会是windows对象(不论是像上边这样赋值给一个变量还是直接在函数内嵌套定义一个函数都是全局对象)
...
1. 如何定义`myCall`?
	通过Function构造函数的原型对象添加方法
2. 如何让函数内部的`this`为某个对象？
	通过动态添加属性的方式将函数绑定到对象上,然后使用delete关键字再把这个新增的函数删除掉就可以了
...
4. 使用[Symbol](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Symbol)调优`myCall`？
	原先的实现中可能会出现动态添加的方法和对象的原方法重名从而导致属性被覆盖,Symbol()方法会返回一个symbol对象,这个对象==可以当做属性的标识(变量名)==,这个方法也可以传入一个参数作为symbol的标识,但是只起标记作用,对代码实际运行没有影响,
	要用symbol对象当做属性名,不能直接使用.语法,而是使用`[]`来动态解析,否则不会正确识别属性名为symbol对象,访问时也是同理
...
console.log('返回值为:', res)
```
>所有方法的原型对象都是一个叫做Function的构造函数
>调用mycall时的this是这个func函数
...
### class核心语法
类是用于创建对象的模版,他们用代码封装数据以处理该数据,js中的类建立在==原型==上
**核心语法:**
...
>实例属性可以用=进行默认值的赋值
>定义的方法是自动创建在==原型对象==上的
>构造函数里可以动态添加属性(不推荐,因为无法直观看到有那些属性)
#### 面试回答:
...
s.sing()
```
>super()==必须==在子类调用this之前以及构造函数return之前
#### 面试回答:
...
```
>私有属性定义和使用时都需要加#,并且访问时都需要通过this
>静态属性==不能通过对象访问==,只能通过类来访问,即使在类内部访问静态变量也需要通过类对象,不能直接使用
>静态方法的this是类对象(和原型对象不是同一个东西),
#### 面试回答:
...
1. class中私有属性/方法
   2. **定义和使用时**需要使用关键字`#`
   3. 私有属性只能在类的内部使用,外部无法使用(代码中)
   4. **Chrome**的控制台中为了方便调试,可以直接访问

5. class中静态属性/方法
   6. **定义和使用时**需要使用关键字`static`
   7. 通过类访问
   8. 静态方法中的`this`是类
...
其背后的基本思路是:不必为了指定子类型的原型而调用超类型的构造函数，我们所需要的无非就是超类型 原型的一个副本而已。
...
```
>Object.create()方法可以基于传入的对象创建一个以这个对象为原型对象的新对象,第二个参数传入的是一个对象,其中是需要修改或添加的属性,为`{<要修改的属性名>:{...},...}`,内部的大括号内可以指定这个属性的一些细节
```js
...
`fetch`：`fetch` 是==浏览器==内置的 API，用于发送网络请求。它提供了一种现代化、基于 Promise 的方式来进行网络通信。用法和`axios`类似，但相比于 `axios`，它的功能和封装级别更为简单。
...
```
>返回的response对象的属性中并没有服务器返回的数据,要想拿到数据需要调用response.json()方法,该方法返回的仍为一个promise对象,需要await拿到返回值,返回值是解析完的对象
...
1. 通过`FormData`添加文件
2. 通过配置项设置,请求方法,请求体(`FormData`不需要设置请求头,会配置默认的请求头)
...
1. 如何定义生成器函数:
	- 在function关键字后加一个*
2. 如何获取`generator`对象
	- 调用这个函数并不会立刻执行而是返回一个generator对象,调用.next()才会真正执行
3. `yield`表达式的使用
	- 会暂停执行并将其后的值作为返回值返回,下次继续执行从上次暂停的地方开始
	- 返回值对象中还有一个done属性标记当前方法是否已经执行完成了
	- 如果已经完成后接着调next(),返回的对象中value为undefined,done为true
4. 通过`for of`可以获取每一个`yield`的值
	- 实际上也是循环调用next并把返回的value赋值给遍历的临时变量,也就是说会触发其中的逻辑代码,而不是只拿返回值
	- 如果当前Generator已经执行完毕,只会循环一次并返回已完成的对象
...
```
>这里如果resolve和reject不是用的箭头函数,则在传入时需要bind进行绑定,否则因为在回调中这两个方法是直接调用的,this会变成window,就无法访问实例对象的属性了,要在传入时绑定到当时的this上
#### 面试回答:
...
2. 判断参数
   ...
   2. 设置默认值
	- onFulfilled的默认值为`(x)=>x`
	- onRejected的默认值为`(x)=>{throw x}`
...
2. `then`方法支持多次调用,即并列的通过一个对象调用多次then方法,而不是链式编程
>多次调用then函数,在promise状态改变后会依次执行回调函数
```javascript
const p = new HMPromise((resolve, reject) => {
  setTimeout(() => {
    resolve('success')
...
2. `queueMicrotask` :新式浏览器均支持,node11开始支持,ie不支持
	- 直接执行一个微任务
3. `MutationObserver` :新式浏览器均支持,ie11开始支持,node不支持
	- 检测DOM节点改变.执行一个异步任务
...
// 这个都熟悉,就不测试了啦
```
用MutationObserver实现queueMicrotask
```js
function myQueueMicrotask(callback) {
  const obs = new MutationObserver(callback)
  const divNode = document.createElement('div')
  obs.observe(divNode,{childList:true})
  divNode.innerText = 'test' //元素发生变化,将回调函数加入微任务队列中
}
```
#### 面试回答
...
![[Pasted image 20251003164005.png]]
...
![[Pasted image 20251003181056.png]]
...
![[Pasted image 20251003185850.png]]
...
2. 内部返回一个新的函数,
   3. 新函数接收需要判断的具体的值
   4. 新函数内部根据外层函数传入的类型,以及传入的值进行判断并返回结果
### 柯里化实际应用-固定参数
...
1. 参数:
   ...
   2. `[wait=0]` *(number)*: 需要延迟的毫秒数。
2. 返回值:
   3. *(Function)*: 返回新的 debounced（防抖动）函数。
#### 核心步骤:
