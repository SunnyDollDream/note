...
属性值：`flex: [flex-grow] [flex-shrink] [flex-basis];`
- **flex-grow**
	定义：项目的放大比例
	参数: number
	作用：当容器有剩余空间时，项目如何分配额外空间
	默认值：0（不放大）
- **flex-shrink**
	定义：项目的缩小比例
	参数: number
...
>flex 元素仅在**默认宽度**之和大于容器的时候才会发生收缩
>默认情况下，元素**不会缩短至小于内容框尺寸**，若想改变这一状况，请设置元素的`min-width` 与 `min-height`属性。
>当一个元素同时被设置了 `flex-basis` (除值为 `auto` 外) 和 `width` (或者在 `flex-direction: column` 情况下设置了`height`) , `flex-basis` 具有更高的优先级。

#### 收缩计算原理
>flex-grow计算同理,如果flex-basis为0则直接按flex-grow比例分配

**总溢出空间** = 容器宽度 - 所有项目的 flex-basis 总和
**收缩权重** = flex-shrink × flex-basis
**每个项目收缩量** = (项目收缩权重 / 总收缩权重) × 总溢出空间
```
/* 假设：
   容器宽度：500px
   项目1：flex: 0 2 200px
...
/* 计算过程：
   ...
   项目2：1 × 300 = 300  
   项目3：3 × 250 = 750
   总权重：400 + 300 + 750 = 1450
   
   收缩量：
   项目1：(400/1450) × 250 ≈ 68.97px
   项目2：(300/1450) × 250 ≈ 51.72px
   项目3：(750/1450) × 250 ≈ 129.31px
   
   最终宽度：
   项目1：200 - 68.97 ≈ 131.03px
   项目2：300 - 51.72 ≈ 248.28px
   项目3：250 - 129.31 ≈ 120.69px
*/
```
#### 全局值
`initial`
