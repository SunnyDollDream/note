...
## React生命周期
>在 Hooks API 中，React 不再暴露“生命周期函数”，而是通过“渲染（render） + 副作用（effect）”来描述组件的整个生命周期。

Hooks 的生命周期可以概括为三件事：
1. **渲染阶段（Render Phase）**
2. **提交阶段（Commit Phase）**
3. **副作用阶段（Effect Phase）**
其中，**`useEffect` 及其变体承担了==全部==生命周期语义**。
...
```
**特点：**
- 每次 state / props 变化都会触发
- **必须是纯计算**
- 不能有副作用（请求、订阅、DOM 操作）
对应理解：
- “函数组件被调用一次 ≈ 发生了一次渲染”
- Hooks 中**没有“只执行一次的 render”**
不能在渲染阶段执行的操作
```js
function Component() {
  // ❌ 不允许
  ...
  // ❌ 不允许
  setState(1); // 即调用useState返回的修改state的方法
...
```
主要是因为 render 阶段可能被 React 多次调用、打断、回滚，但这些操作不能被回滚
因此 React 要求：
- render 必须是 **纯函数**
- **同样的 props + state → 同样的 UI**
### 提交（Commit）
React 将本次 render 结果：
- 应用到真实 DOM
- 更新 refs
- 准备执行副作用
这是 **React 内部阶段**，开发者不可直接控制。
### 副作用（Effects）——生命周期的核心
`useEffect` —— 默认生命周期 Hook
```js
useEffect(() => {
  // 副作用逻辑
  return () => {
...
|首次执行|组件挂载完成后|
|依赖变化|组件更新完成后|
|返回函数|下次 effect 执行前 / 组件卸载时|
>类比ClassAPI时的生命周期,useEffect = didMount + didUpdate + willUnmount
1. 挂载（Mount）
```js
useEffect(() => {
  console.log('mounted');
...
- 浏览器绘制前
- 会阻塞绘制
2. `useInsertionEffect`（进阶）
- 用于 CSS-in-JS
- 在 DOM 变更前执行
- 一般业务开发**不使用**
## navigator.sendBeacon
**`sendBeacon` 是浏览器提供的一个“低优先级、可靠上报数据”的接口**，主要用于：在页面卸载、跳转、关闭时，仍然尽可能把数据发送到服务器
### 用法
```js
...
- 返回值：`boolean`
    - `true`：浏览器已接收并尝试发送
    - `false`：浏览器拒绝（通常是参数不合法）
>`true ≠ 服务端成功接收`，只是“浏览器已经接管发送”。
### 核心特性
...
- **只适合小数据、低频上报**
### Blob
**`Blob`（Binary Large Object）表示一段不可变的二进制数据**，是浏览器中的“通用数据容器”。
```js
const blob = new Blob(
  [data], 
...
```
- 第一个参数：数据片段数组（字符串 / ArrayBuffer 等）
- 第二个参数：MIME 类型（非常重要）
### sendBeacon 和 fetch / axios 的区别

|对比点|sendBeacon|fetch / axios|
|---|---|---|
|页面卸载|✅ 可发送|❌ 易被取消|
|响应处理|❌ 不可|✅|
|自定义 headers|❌ 不支持|✅|
