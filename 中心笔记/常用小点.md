...
**session = HttpSession**
- 由 **服务器端** 创建和维护
- 每个用户（浏览器）通常对应一个 session
- 用来在**多次 HTTP 请求之间保存数据**
##### 场景
- 登录状态保存
- 用户信息缓存
- 验证码校验
...
##### 使用
获取:
> 在Spring中也可以用原生方法获取,只是你放在参数里Spring会自动传入
```Java
// 原生
...
> 这里的request是servlet在请求到达时自动创建的HttpServletRequest对象(也不允许手动new),在Spring中可以通过在controller参数中写上以自动传入,要自己获取的话可以通过如下方式
...
```
##### 原理
服务端调用request.getSession()时,先根据JSESSIONID查看在内存中是否存在对应的session,没有则创建并分配JSESSIONID,之后servlet容器会在响应中自动带上`Set-Cookie: JSESSIONID=A1B2C3D4; Path=/; HttpOnly`响应头,即返回浏览器一个包含JSESSIONID的cookie,之后再请求时就会带着这个cookie,这样服务器就可以根据SESSIONID获取对应的session了,然后再从session中获取对应的状态数据即可